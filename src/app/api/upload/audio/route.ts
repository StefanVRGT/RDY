  ...
  return new Promise<NextResponse>((resolve) => {
    ...
    bb.on('file', (_field, stream, info) => {
      if (!ALLOWED_MIME_TYPES.includes(info.mimeType)) {
        errorMsg = 'Invalid file type. Only audio files (MP3, WAV, OGG, WebM) are allowed.';
        stream.resume();
        return;
      }

      mimeType = info.mimeType;
      const ext = info.mimeType === 'audio/webm' ? 'webm' : info.mimeType.split('/')[1].replace('mpeg', 'mp3');
