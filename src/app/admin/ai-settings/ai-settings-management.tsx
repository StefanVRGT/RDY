...
export function AISettingsManagement() {
    ...
    return (
      <div className="space-y-6">
        <Card className="border-rdy-gray-200 bg-rdy-gray-100">
          <CardContent className="py-8 text-center text-rdy-gray-400">Loading...</CardContent>
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-rdy-gray-200 bg-rdy-gray-100">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-rdy-black">AI Features</CardTitle>
      ...
      <Card className="border-rdy-gray-200 bg-rdy-gray-100">
        <CardHeader>
          <CardTitle className="text-rdy-black">Default Provider</CardTitle>
          <CardDescription className="text-rdy-gray-400">
        ...
        <CardContent>
          ...
          >
            ...
            <SelectContent className="border-rdy-gray-200 bg-rdy-gray-100">
              <SelectItem value="anthropic" className="text-rdy-black">
                Anthropic (Claude)
              </SelectItem>
              <SelectItem value="gemini" className="text-rdy-black">
                Google (Gemini)
              </SelectItem>
            </SelectContent>
          </Select>
      ...
      <Card className="border-rdy-gray-200 bg-rdy-gray-100">
        <CardHeader>
          <CardTitle className="text-rdy-black">API Keys</CardTitle>
          <CardDescription className="text-rdy-gray-400">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="rounded-lg border border-rdy-gray-200 bg-rdy-gray-100 p-4">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="font-medium text-rdy-black">Anthropic API Key</h4>
              ...
              <div className="flex gap-2">
                {settings.hasAnthropicKey && (
                  <Button
                    variant="secondary"
                    size="sm"
                    className="bg-rdy-gray-200 text-rdy-black hover:bg-rdy-gray-200"
                    onClick={() => {
                      setTestResult(null);
          ...
          <div className="rounded-lg border border-rdy-gray-200 bg-rdy-gray-100 p-4">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="font-medium text-rdy-black">Gemini API Key</h4>
      ...
      <Card className="border-rdy-gray-200 bg-rdy-gray-100">
        <CardHeader>
          <CardTitle className="text-rdy-black">Model Configuration</CardTitle>
          <CardDescription className="text-rdy-gray-400">
        ...
        <CardContent className="space-y-4">
          {TASK_TYPES.map((taskType) => {
            ...
            return (
              <div
                key={taskType}
                className="flex items-center justify-between rounded-lg border border-rdy-gray-200 bg-rdy-gray-100 p-4"
              >
                <div>
                  <h4 className="font-medium text-rdy-black">{TASK_TYPE_LABELS[taskType].label}</h4>
                  <p className="text-sm text-rdy-gray-400">{TASK_TYPE_LABELS[taskType].description}</p>
                  {config ? (
                    <p className="mt-1 text-sm text-green-400">
                      {config.provider === 'anthropic' ? 'Anthropic' : 'Gemini'}: {config.model}
                    </p>
                  ) : (
                    <p className="mt-1 text-sm text-rdy-gray-500">Using default provider</p>
                  )}
                </div>
                <div className="flex gap-2">
                  <Button
        ...
        <DialogContent className="border-rdy-gray-200 bg-white">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <label htmlFor="apiKey" className="text-sm text-rdy-gray-600">
                API Key
              </label>
              <Input
                id="apiKey"
      ...
      <Card className="border-rdy-gray-200 bg-rdy-gray-100">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-rdy-black">Usage Statistics</CardTitle>
              <CardDescription className="text-rdy-gray-400">
            ...
            >
              <SelectTrigger className="w-[150px] border-rdy-gray-200 bg-rdy-gray-100 text-rdy-black">
                <SelectValue placeholder="Select period" />
              </SelectTrigger>
              <SelectContent className="border-rdy-gray-200 bg-rdy-gray-100">
                <SelectItem value="7" className="text-rdy-black">Last 7 days</SelectItem>
                <SelectItem value="30" className="text-rdy-black">Last 30 days</SelectItem>
                <SelectItem value="90" className="text-rdy-black">Last 90 days</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </CardHeader>
          ...
          {isLoadingStats ? (
            ...
            <>
              ...
              <div className="grid gap-4 md:grid-cols-4">
                <div className="rounded-lg border border-rdy-gray-200 bg-rdy-gray-100 p-4">
                  <p className="text-sm text-rdy-gray-400">Total Requests</p>
                  <p className="text-2xl font-bold text-rdy-black">
                    {usageStats.totals.totalRequests.toLocaleString()}
                  </p>
                </div>
                <div className="rounded-lg border border-rdy-gray-200 bg-rdy-gray-100 p-4">
                  <p className="text-sm text-rdy-gray-400">Total Tokens</p>
                  <p className="text-2xl font-bold text-rdy-black">
                    {usageStats.totals.totalTokens.toLocaleString()}
                  </p>
                </div>
                <div className="rounded-lg border border-rdy-gray-200 bg-rdy-gray-100 p-4">
                  <p className="text-sm text-rdy-gray-400">Estimated Cost</p>
                  <p className="text-2xl font-bold text-rdy-black">
                    ${(usageStats.totals.totalCostCents / 100).toFixed(2)}
                  </p>
                </div>
                <div className="rounded-lg border border-rdy-gray-200 bg-rdy-gray-100 p-4">
                  <p className="text-sm text-rdy-gray-400">Success Rate</p>
                  <p className="text-2xl font-bold text-rdy-black">
              ...
              {usageStats.byProvider.length > 0 && (
                <div>
                  <h4 className="mb-3 font-medium text-rdy-black">Usage by Provider</h4>
                  <div className="grid gap-4 md:grid-cols-2">
                    {usageStats.byProvider.map((stat) => (
                      <div
                        key={stat.provider}
                        className="rounded-lg border border-rdy-gray-200 bg-rdy-gray-100 p-4"
                      >
                        <div className="flex items-center justify-between">
                          <h5 className="font-medium text-rdy-black">
                        ...
                        <div className="mt-2 grid grid-cols-3 gap-2 text-sm">
                          <div>
                            <p className="text-rdy-gray-400">Tokens</p>
                            <p className="text-rdy-black">{(stat.totalTokens || 0).toLocaleString()}</p>
                          </div>
                          <div>
                            <p className="text-rdy-gray-400">Cost</p>
                            <p className="text-rdy-black">${((stat.totalCostCents || 0) / 100).toFixed(2)}</p>
                          </div>
                          <div>
                            <p className="text-rdy-gray-400">Errors</p>
              ...
              {usageStats.byTaskType.length > 0 && (
                <div>
                  <h4 className="mb-3 font-medium text-rdy-black">Usage by Task Type</h4>
                  <Table>
                    <TableHeader>
                      <TableRow className="border-rdy-gray-200">
                        <TableHead className="text-rdy-gray-400">Task Type</TableHead>
                        <TableHead className="text-right text-rdy-gray-400">Requests</TableHead>
                        <TableHead className="text-right text-rdy-gray-400">Tokens</TableHead>
                        <TableHead className="text-right text-rdy-gray-400">Cost</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {usageStats.byTaskType.map((stat) => (
                        <TableRow key={stat.taskType} className="border-rdy-gray-200">
                          <TableCell className="text-rdy-black">
                            {TASK_TYPE_LABELS[stat.taskType as AITaskType]?.label || stat.taskType}
                          </TableCell>
                          <TableCell className="text-right text-rdy-black">
              ...
              {usageStats.recentRequests.length > 0 && (
                <div>
                  <h4 className="mb-3 font-medium text-rdy-black">Recent Requests</h4>
                  <Table>
                    <TableHeader>
                      <TableRow className="border-rdy-gray-200">
                        <TableHead className="text-rdy-gray-400">Time</TableHead>
                        <TableHead className="text-rdy-gray-400">Provider</TableHead>
                        <TableHead className="text-rdy-gray-400">Model</TableHead>
                        <TableHead className="text-rdy-gray-400">Task</TableHead>
                        <TableHead className="text-right text-rdy-gray-400">Tokens</TableHead>
                        <TableHead className="text-right text-rdy-gray-400">Status</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {usageStats.recentRequests.map((req) => (
                        <TableRow key={req.id} className="border-rdy-gray-200">
                          ...
                          <TableCell className="text-rdy-gray-600">{req.model}</TableCell>
                          <TableCell className="text-rdy-gray-600">
      ...
      <Dialog open={showModelConfigDialog} onOpenChange={setShowModelConfigDialog}>
        <DialogContent className="border-rdy-gray-200 bg-white">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <label className="text-sm text-rdy-gray-600">Provider</label>
              <Select
            ...
            <div className="space-y-2">
              <label className="text-sm text-rdy-gray-600">Model</label>
              <Select value={selectedModel} onValueChange={setSelectedModel}>
                ...
                <SelectContent className="border-rdy-gray-200 bg-rdy-gray-100">
                  {availableModels?.map((model) => (
                    <SelectItem key={model.id} value={model.id} className="text-rdy-black">
                      <div>
                        <div>{model.name}</div>
                        <div className="text-xs text-rdy-gray-400">{model.description}</div>
                      </div>
                    </SelectItem>
                  ))}
                </SelectContent>
