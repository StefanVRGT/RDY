...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-rdy-black">Dashboard</h1>
        <p className="mt-1 text-rdy-gray-400">Overview of your organization</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="border-rdy-gray-200 bg-rdy-gray-100">
          <CardHeader className="pb-2">
            <CardDescription className="text-rdy-gray-400">Mentors</CardDescription>
            <CardTitle className="text-3xl text-rdy-black">
              {statsLoading ? (
                <span className="inline-block h-8 w-16 animate-pulse rounded bg-rdy-gray-200" />
              ) : (
        ...
        <Card className="border-rdy-gray-200 bg-rdy-gray-100">
          <CardHeader className="pb-2">
            <CardDescription className="text-rdy-gray-400">Mentees</CardDescription>
            <CardTitle className="text-3xl text-rdy-black">
              {statsLoading ? (
                <span className="inline-block h-8 w-16 animate-pulse rounded bg-rdy-gray-200" />
              ) : (
        ...
        <Card className="border-rdy-gray-200 bg-rdy-gray-100">
          <CardHeader className="pb-2">
            <CardDescription className="text-rdy-gray-400">Active Classes</CardDescription>
            <CardTitle className="text-3xl text-rdy-black">
              {statsLoading ? (
                <span className="inline-block h-8 w-16 animate-pulse rounded bg-rdy-gray-200" />
              ) : (
        ...
        <Card className="border-rdy-gray-200 bg-rdy-gray-100">
          <CardHeader className="pb-2">
            <CardDescription className="text-rdy-gray-400">Pending Invitations</CardDescription>
            <CardTitle className="text-3xl text-rdy-black">
              {statsLoading ? (
                <span className="inline-block h-8 w-16 animate-pulse rounded bg-rdy-gray-200" />
              ) : (
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="border-rdy-gray-200 bg-rdy-gray-100">
          <CardHeader>
            <CardTitle className="text-rdy-black">Quick Actions</CardTitle>
        ...
        <Card className="border-rdy-gray-200 bg-rdy-gray-100 lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-rdy-black">Recent Activity</CardTitle>
            <CardDescription className="text-rdy-gray-400">
          ...
          <CardContent>
            {activitiesLoading ? (
              <div className="space-y-4">
                {[...Array(5)].map((_, i) => (
                  <div key={i} className="flex items-center gap-4">
                    <div className="h-8 w-8 animate-pulse rounded-full bg-rdy-gray-200" />
                    <div className="flex-1 space-y-2">
                      <div className="h-4 w-3/4 animate-pulse rounded bg-rdy-gray-200" />
                      <div className="h-3 w-1/4 animate-pulse rounded bg-rdy-gray-200" />
                    </div>
                  </div>
                ))}
              </div>
                ...
                {activities.map((activity) => (
                  <div key={activity.id} className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-rdy-black truncate">{activity.description}</p>
                      <p className="text-xs text-rdy-gray-500">{formatRelativeTime(activity.timestamp)}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-col items-center justify-center py-8 text-center">
                <svg className="h-12 w-12 text-rdy-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                </svg>
                <p className="mt-2 text-sm text-rdy-gray-500">No recent activity</p>
                <p className="text-xs text-rdy-gray-400">Activity will appear here as users join and classes are created</p>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      <Card className="border-rdy-gray-200 bg-rdy-gray-100">
        <CardHeader>
          <CardTitle className="text-rdy-black">User Summary</CardTitle>
          <CardDescription className="text-rdy-gray-400">
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3">
            <div className="rounded-lg border border-rdy-gray-200 bg-rdy-gray-100 p-4">
              ...
              <div className="mt-2 h-2 overflow-hidden rounded-full bg-rdy-gray-200">
          ...
          {stats?.users.newLast7Days !== undefined && stats.users.newLast7Days > 0 && (
            <p className="mt-4 text-sm text-rdy-gray-500">
